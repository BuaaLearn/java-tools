...
public class ChineseUtil {
     ...
     * @return
     */
    private static Set<Character> extractCharacter(int start, int step, List<String> lines) {
    ...
    public static double transCnToDouble(String text, double quantifier) {
        ...
        if ((pos = text.indexOf("亿")) != -1) {
            amounts =
                    transCnToDouble(text.substring(0, pos), 10) * 100000000
                            + transCnToDouble(text.substring(pos + 1), 100000000);
        } else if ((pos = text.indexOf("万")) != -1) {
            amounts =
                    transCnToDouble(text.substring(0, pos), 10) * 10000
                            + transCnToDouble(text.substring(pos + 1), 10000);
        } else {
